...
| `-car, --cache-after-resp [N]` | create a context checkpoint every N completed responses (bare option = 1, 0 = disabled, default: disabled). Retains checkpoints until `--ctx-checkpoints` is full, then evicts the least recently restored checkpoint.<br/>(env: LLAMA_ARG_CACHE_AFTER_RESP) |
