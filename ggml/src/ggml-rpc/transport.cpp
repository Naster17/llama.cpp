...
static void set_large_bufs(sockfd_t sockfd) {
    int size = 4 * 1024 * 1024;
    if (const char * env = getenv("GGML_RPC_TCP_BUFFER_SIZE")) {
        long val = atol(env);
        if (val >= 64 * 1024 && val <= 64 * 1024 * 1024) {
            size = (int) val;
        }
    }
...
socket_ptr socket_t::accept() {
    ...
    set_large_bufs(client_socket_fd);
