...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--cache-device"},
        "keep response checkpoints and prompt-cache states in backend buffers (default: disabled)",
        [](common_params & params) {
            params.cache_device = true;
        }
    ).set_env("LLAMA_ARG_CACHE_DEVICE").set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-kvu", "--kv-unified"},
